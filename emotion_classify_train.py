...
def set_args(filename):
    ...
    parser.add_argument("--train_data_filename",
                        default='data/emt_data/train.csv',
                        type=str,
    ...
    parser.add_argument("--test_data_filename",
                        default='data/emt_data/test.csv',
                        type=str)
    parser.add_argument("--train_data_dir",
                        default='data/emt_data/',
                        type=str,
    ...
    parser.add_argument("--mymodel_config_dir",
                        default='config/emotion_classify_config.json',
                        type=str)
    parser.add_argument("--mymodel_save_dir",
                        default='checkpoint/emotion_classify/',
                        type=str)
    ...
    parser.add_argument("--emt2label",
                        default={'Good': 1, 'Bad': 0},
                        type=list)
    parser.add_argument("--label2emt",
                        default={1: 'Good', 0: 'Bad'},
                        type=list)
...
# 设置全局环境
try:
    args = set_args('config/emotion_classify_args.txt')
...
# emtion转label
def emt2label(t_label, args):
    try:
        return args.emt2label[t_label]
    except:
        return len(args.emt2label)-1
...
def get_dataloader(filenames):
    ...
    with open(filenames, encoding='utf-8')as f:
        f_csv = csv.reader(f)
        for row in f_csv:
            tmp, _, _ = text2ids(tokenizer, row[1], args.max_sent_len)
...
def main():
    ...
    mymodel_pred(logger, input('请输入一句话，我给出情感:'))
